...
export function topbarVars(topbar: { style?: string; accent?: boolean; ornament?: string; height?: number }, mode: 'light' | 'dark'): string {
  ...
  if (style === 'solid') {
    ...
    bg = 'linear-gradient(180deg, color-mix(in srgb, var(--primary) 16%, var(--bg)), var(--bg) 72%)'
    blur = '0px'
  } else {
    bg = 'color-mix(in srgb, var(--bg) 82%, transparent)'
    blur = '12px'
  }
  if (accent) {
  ...
  return `--topbar-bg: ${bg};--topbar-blur: ${blur};--topbar-line: ${line};--topbar-ornament-src: ${orn};--topbar-height: ${height}px;`
}
