...
import { Archive, Download, FileArchive, HardDriveDownload, RefreshCw, Trash2 } from 'lucide-react'
...
export default function BackupPage() {
  ...
  const [config, setConfig] = useState({ intervalHours: 24, keep: 5, enabled: true })
  ...
  async function run(action: 'backup' | 'export') {
    setRunning(action)
    try {
      await api('/admin/api/backup', { method: 'POST', body: JSON.stringify({ action }) })
  ...
  async function saveConfig() {
    try {
      await api('/admin/api/backup', {
        method: 'PUT',
        body: JSON.stringify({ ...config, intervalHours: Number(config.intervalHours), keep: Number(config.keep) }),
      })
      toast.success('已保存')
      await load()
    } catch (e) {
    ...
    <div className="mx-auto flex max-w-4xl flex-col gap-4">
      ...
      <Card className="gap-4 p-5 sm:p-6">
        ...
        <p className="text-xs text-muted-foreground">
          完整备份 = SQLite 配置 + 上传资产 + 全部内容文件;内容导出 = 纯内容包,可直接合并到静态版仓库。
        </p>
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" onClick={() => run('export')} disabled={running !== null}>
            <FileArchive />
      ...
      <Card className="gap-0">
        <div className="border-b px-5 py-3.5">
          <h3 className="text-sm font-semibold">备份列表({state?.backups.length ?? 0})</h3>
        </div>
        <div className="flex flex-col divide-y">
          {!state ? (
            <Skeleton className="m-4 h-24" />
          ) : state.backups.length === 0 ? (
            <p className="p-8 text-center text-sm text-muted-foreground">还没有备份,点上方「立即备份」</p>
          ) : (
            state.backups.map((b) => (
              <div key={b.name} className="flex items-center gap-3 px-5 py-3">
                ...
                <Button variant="ghost" size="icon" className="text-destructive" onClick={() => setDeleteTarget(b)} title="删除">
                  <Trash2 />
                </Button>
              </div>
            ))
      ...
      <Card className="gap-0">
        <div className="border-b px-5 py-3.5">
          <h3 className="text-sm font-semibold">内容导出({state?.exports.length ?? 0})</h3>
        </div>
        <div className="flex flex-col divide-y">
          {!state ? (
            <Skeleton className="m-4 h-24" />
          ) : state.exports.length === 0 ? (
            <p className="p-8 text-center text-sm text-muted-foreground">还没有导出</p>
          ) : (
            state.exports.map((b) => (
              <div key={b.name} className="flex items-center gap-3 px-5 py-3">
      ...
      <AlertDialog open={deleteTarget !== null} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>「{deleteTarget?.name}」将被永久删除。</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
