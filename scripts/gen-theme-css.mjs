...
const defaultId = 'gray'
const defaultPreset = presets.find((p) => p.id === defaultId) ?? presets[0]

function emitVars(prefix, entries) {
  const lines = entries.map(([k, v]) => `  --${k}: ${v};`).join('\n')
  return `${prefix} {\n${lines}\n}\n`
}

let css = `/* AUTO-GENERATED by scripts/gen-theme-css.mjs — 请勿手改 */\n/* 修改 src/themes/presets/*.json 后运行 npm run gen:themes */\n\n/* 默认主题兜底变量(防止首帧变量未定义导致顶部栏背景透明/主题色丢失) */\n`

// 输出 :root 兜底变量(取默认主题的 light 模式,后续 [data-theme][data-mode] 会覆盖)
if (defaultPreset) {
  const lightPalette = defaultPreset.palette.light
  css += emitVars(':root', [
    ['wallpaper-light-src', defaultPreset.wallpaper.light.startsWith('gradient:') ? defaultPreset.wallpaper.light.slice(9) : defaultPreset.wallpaper.light],
    ['wallpaper-dark-src', defaultPreset.wallpaper.dark.startsWith('gradient:') ? defaultPreset.wallpaper.dark.slice(9) : defaultPreset.wallpaper.dark],
    ['mascot-primary', defaultPreset.mascot.primary],
    ['mascot-secondary', defaultPreset.mascot.secondary],
    ...Object.entries(lightPalette),
  ])
  css += `:root[data-mode="light"] {\n${topbarVars(defaultPreset.topbar, 'light')}\n}\n`
  css += `:root[data-mode="dark"] {\n${topbarVars(defaultPreset.topbar, 'dark')}\n}\n`
}
