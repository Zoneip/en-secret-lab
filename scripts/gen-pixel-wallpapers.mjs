...
function friendsShrineNight() {
  ...
  // 巫女(夜:剪影 + 月光描边 + 提灯)
  const miko = G() +
    R(202, 118, 3, 3, '#2A2236') + R(203, 121, 3, 2, '#241E30') + R(204, 123, 3, 3, '#1E1828') + R(205, 126, 3, 4, '#1E1828') + R(204, 130, 3, 4, '#241E30') +
    R(201, 120, 2, 2, '#362C44') +
    R(198, 116, 3, 2, '#A03C52') + R(199, 118, 2, 2, '#B84A60') + R(198, 114, 2, 2, '#4A3E58') +
    R(201, 119, 4, 4, '#5A4E66') + R(203, 120, 1, 1, '#1E1828') + R(201, 122, 2, 1, '#4A3E56') +
    // 白衣:月光下的淡紫白(保持巫女白衣设定,仅随夜色降明度)
    R(199, 123, 8, 9, '#9C92A8') + R(200, 123, 2, 2, '#B04A60') + R(205, 123, 2, 2, '#B04A60') +
    R(200, 126, 1, 3, '#B4AABE') + R(205, 126, 1, 3, '#B4AABE') +
    R(198, 132, 10, 2, '#A83A50') + R(199, 134, 8, 1, '#8E3044') +
    R(197, 135, 12, 10, '#8E3450') + R(199, 137, 8, 2, '#A23E5A') +
    R(198, 143, 10, 5, '#7A2C44') + R(200, 145, 4, 3, '#662438') +
    R(200, 136, 1, 8, '#7A2C44') + R(204, 136, 1, 8, '#7A2C44') +
    R(194, 124, 5, 9, '#9C92A8', 'sleeveL') + R(196, 130, 3, 4, '#8E8499', 'sleeveL') +
    R(207, 124, 5, 9, '#9C92A8', 'sleeveR') + R(209, 130, 3, 4, '#8E8499', 'sleeveR') +
    // 月光描边(发顶/肩)
    R(202, 117, 1, 1, '#8E84A0') + R(199, 123, 8, 1, '#B0A6BC') +
    // 提灯(暖光)
